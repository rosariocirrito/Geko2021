...
<h1 id="titolo-lista">Programmazione Obiettivi Apicali per dipartimento: <em>${struttura} </em>- anno ${anno}</h1>
<hr />  
...
<c:forEach items="${listObiettivi}" var="obj" varStatus ="status">
	...
	<a href="<spring:url value="/controllerAssociaProgramma/new/${obj.idObiettivo}/${idIncaricoApicale}" htmlEscape="true" />">ASSOCIA PROGRAMMA </a>
	
   </c:if>
		<c:forEach items="${obj.associazProgramma}" var = "associaz">
		<a href="<spring:url value="/controllerAssociaProgramma/${associaz.id}/edit/${idIncaricoApicale}" htmlEscape="true" />">MODIFICA </a>
...
<table class="footer" >
    ...
    <tr>
      <td>[view: associaProgrammaIncaricoApicaleController.jsp 2019/05/27] </td>
    </tr>
</table>
</div>
