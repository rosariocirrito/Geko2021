...
<c:forEach items="${listAreeStrategiche}" var="area">
	...
	<tbody>   
        ...
        <c:forEach items="${area.prioritaPolitiche}" var="prio" varStatus ="status">
	        ...
	        <tr>
	            <td> 
	                <h5>${objStrat.descrizione}</h5>
	                <table>
	                <tbody>
	                <c:forEach items="${objStrat.associazObiettivi}" var="associaz" varStatus ="statusAssociaz">
			        <tr>
			        <td> 
			                <h6>${associaz.apicale.dipartimento.codice} - ${associaz.apicale.struttura.codice} - ${associaz.apicale.descrizione}</h6>
			            </td>
			        </tr>
			        </c:forEach>
	                </tbody>
...
<table class="footer" >
    ...
    <tr>
	  	<td>[view:listDirettivaSepicos.jsp] </td>
	</tr>
  </table>
</div>
