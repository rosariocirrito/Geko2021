...
<h1 id="titolo-lista">Programmazione Obiettivi per struttura: <em>${struttura} </em>- anno ${anno}</h1>
<hr />  
...
<c:forEach items="${listObiettivi}" var="obj" varStatus ="status">	
	...
	<td>
		...
		<c:forEach items="${obj.associazObiettivi}" var = "associaz">
			<a href="<spring:url value="/managerAssocia/${associaz.id}/edit/${idIncarico}" htmlEscape="true" />">MODIFICA </a>
   ...
   <c:if test="${empty obj.associazObiettivi}">    
    <a href="<spring:url value="/managerAssocia/new/${obj.idObiettivo}/${idIncarico}" htmlEscape="true" />">ASSOCIA STRATEGICO </a>
	</c:if> 
...
<table class="footer" >
    ...
    <tr>
      <td>[view: associaPianificazioneStrategicaIncaricoManager.jsp 2019/05/24] </td>
    </tr>
</table>
</div>
