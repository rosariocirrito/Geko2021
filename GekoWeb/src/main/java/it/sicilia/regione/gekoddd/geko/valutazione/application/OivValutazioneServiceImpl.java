 ...
 * @author Cirrito
 */
@Service("OivValutazioneService")
public class OivValutazioneServiceImpl implements OivValutazioneService{

	private Log log = LogFactory.getLog(OivValutazioneServiceImpl.class);
...
// ------- Casi d'uso del controller -----------------------------------------
 ...
 // -----------------------------------------------------------------------
    @Transactional
    @Secured({"ROLE_OIV"})
    public void oivCreateValutazione(Valutazione valutazione) {
    	...
    	final PersonaGiuridicaGeko pg = fromOrganikoServizi.findPersonaGiuridicaById(incarico.pgID);
	        ...
	        journal.setPerche("oivCreateValutazione");
    ...
    @Transactional
    @Secured({"ROLE_OIV"})
    public void oivUpdateValutazione(Valutazione valutazione) {
    	final IncaricoGeko incarico = fromOrganikoServizi.findIncaricoById(valutazione.getIncaricoID());
    	final PersonaFisicaGeko pf = fromOrganikoServizi.findPersonaFisicaById(incarico.pfID);
    	final PersonaGiuridicaGeko pg = fromOrganikoServizi.findPersonaGiuridicaById(incarico.pgID);
    		valutazioneCmdServizi.save(valutazione);
	        ...
	        journal.setDove("Valutazione");
	        journal.setIdObj(valutazione.getId());
	        journal.setPerche("oivUpdateValutazione");
	        journal.setQuando(new Date());
	        journalServizi.save(journal);
    ...
    @Transactional
    @Secured({"ROLE_OIV"})
    public void oivCreateValutazioneCompOrg(Valutazione valutazione) {
    ...
    @Transactional
    @Secured({"ROLE_OIV"})
    public void oivUpdateValutazioneCompOrg(Valutazione valutazione) {
    	final IncaricoGeko incarico = fromOrganikoServizi.findIncaricoById(valutazione.getIncaricoID());
