...
public class ManagerProgrPdfController  {
	//
    @Autowired
    ...
    //
    public ManagerProgrPdfController() { }
 ...
 // 1. lista obiettivi e azioni e dipendenti in base al nome del dirigente
    @RequestMapping(value="pdfPianificazioneIncaricoManager/{anno}/{idIncarico}")
    public void pdfPianificazioneManager(@PathVariable("anno") int anno, @PathVariable("idIncarico") int idIncarico,
    	...
    	} // fine metodo pdfPianificazioneIncaricoManager -----------------------------------------------------------------
  ...
  //----------------------- Valutazione --------------------
    ...
    @RequestMapping(value="pdfProgrammazioneCompartoManager/{anno}/{idIncarico}")
    ...
    // *********************** 
    // 2. lista obiettivi e azioni e dipendenti in base al nome del dirigente
    @RequestMapping(value="pdfProgrammazioneDipartimentaleCompartoManager/{anno}/{idIncarico}")
    public void pdfProgrammazioneDipartimentaleCompartoManager(@PathVariable("anno") int anno, @PathVariable("idIncarico") int idIncarico,
    		HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	// cfr iText in Action p.285 e 289
    	try {
    		String text = request.getParameter("text");
 ...
 // **************************** METODI PRIVATI **********************************************************************
    ...
    private void generaSchedaObiettivi(Document doc, IncaricoGeko inc, IncaricoGeko incaricoApicale, int anno){
    	...
    	try {
			tableSk1.setWidths(new int[] {1,5,1,2,2,2,1});
		} catch (DocumentException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
 ...
 // ------------------------------------------------------------------------------------
  ...
  //-----------------------Pianificazione Dipartimentale ------------ 2019  --------
    ...
    @RequestMapping(value="pdfProgrammazionePopManager/{anno}/{idIncarico}")
    public void pdfProgrammazioneDipartimentaleController(@PathVariable("anno") int anno, @PathVariable("idIncarico") int idIncarico,
    		HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // scrivo sulla prima pagina il titolo del report
		Paragraph titolo = new Paragraph("PROGRAMMAZIONE POP PER STRUTTURA",fontTitle1);
		titolo.setAlignment("CENTER");
     	doc.add(titolo);
     	// Riga vuota
	    doc.add(new Paragraph("\n"));
	    ...
	    if (null != incarico.responsabile) {
	    	...
	    	titolo2 = new Paragraph("RESPONSABILE STRUTTURA DA INSERIRE !!! (manca incarico apicale");
	    }
	    titolo2.setAlignment("CENTER");
     	doc.add(titolo2);
     	// Riga vuota
		...
		// verifico se lista vuota
	    if (lstIncPop.isEmpty()){
	    	Paragraph empty = new Paragraph("Nessun Incarico POP trovato per "+incarico.denominazioneStruttura+" - anno:"+anno);
	     	doc.add(empty);
	     	doc.add(new Paragraph("\n"));
	    }
	    ...
	    for (IncaricoGeko inc : lstIncPop) {	
	    	...
	    	// scheda 1
	    	this.generaSchedaObiettiviPop(doc, inc,incarico,anno);
			//this.addFirme(doc,incaricoApic,inc);				    
			    	
		    // schede 2	    	
	    	// itero sugli obiettivi
		    this.generaSchedeDettaglioObiettiviPop(doc, inc,incarico,anno);
		    //this.addFirme(doc,incaricoApic,inc);
			    //
	    // ------------------------------
	    } // fine loop incarico
    ...
    } // fine metodo pdfProgrammazionePopManager ---------------------------------------
    
 // ------------------------------------------------------------------------------------
    private void generaSchedeDettaglioObiettiviPop(Document doc, IncaricoGeko inc, IncaricoGeko incaricoDirigenziale, int anno){
    	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yy"); 
    	int nordObj = 0;
    	for(Obiettivo obj : inc.getObiettivi()){
    		//
		    ...
		    this.addFirme(doc, "IL DIRIGENTE","IL RESPONSABILE POP",incaricoDirigenziale.getResponsabile(),inc.getResponsabile());
    	} // fine for obiettivi    	
    }
 
    
 // -------------------------------------------------------------------------------------
    private void generaSchedaObiettiviPop(Document doc, IncaricoGeko inc, IncaricoGeko incaricoDirigenziale, int anno){
    	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yy"); 
    	//
		Font fontTitle3 = new Font();
		fontTitle3.setFamily("HELVETICA");
    	...
    	tableProgr.getDefaultCell().setBorder(PdfPCell.NO_BORDER);
    	tableProgr.getDefaultCell().setCellEvent(whiteRectangle);
    	PdfPCell cellriga1 = new PdfPCell(new Paragraph("SCHEDA PROGRAMMAZIONE 1: programmazione obiettivi del responsabile ai fini della successiva valutazione della performance - Anno "+anno,fontTitle3));
    	tableProgr.addCell(cellriga1);    	
    	...
    	PdfPCell cellResp = new PdfPCell(new Paragraph("Titolare Posizione: "+nomeResponsabile));
    	tableProgr.addCell(cellResp);
    	//PdfPCell competenze = new PdfPCell(new Paragraph(mapItem.getKey().getCompetenze()));
    	//tableStru.addCell(competenze);
    	//
    	...
    	// genero la scheda 1
    	int nrObj = 0;
    	PdfPTable tableSk1 = new PdfPTable(6);
    	try {
			tableSk1.setWidths(new int[] {1,5,2,2,2,1});
		} catch (DocumentException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		for (Valutazione valutazione : inc.getValutazioni()){
			...
			if (valutazione.getAnno() == anno){
				// tabella attuazione piano di lavoro
				PdfPTable tableValutaz = new PdfPTable(2);
				// define relative columns width p.96
				try {
					tableValutaz.setWidths(new int[] {4,1});
				} catch (DocumentException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
