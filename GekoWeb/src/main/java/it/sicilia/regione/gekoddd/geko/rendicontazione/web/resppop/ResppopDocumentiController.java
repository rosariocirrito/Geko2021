...
import it.sicilia.regione.gekoddd.geko.rendicontazione.application.ResppopRendicontazioneService;
...
@RequestMapping("/respPopDocumenti")
public class ResppopDocumentiController  {
	
	private Log log = LogFactory.getLog(ResppopDocumentiController.class);
    ...
    @Autowired
    private ResppopRendicontazioneService resppopServizi;
    
    @Autowired
    private Menu menu;
    
    //
    public ResppopDocumentiController() { }
    ...
    public String updateForm(@PathVariable("id") int id, Model model) {
		...
		if (menu.getIdIncaricoScelta() == documento.getIncaricoID()){
	            ...
	            return "geko/rendicontazione/resppop/formDocumentoEditPop";
	        }
    ...
    public String processUpdateSubmit(@ModelAttribute Documento documento, 
		...
		if (result.hasErrors()) {
            ...
            this.resppopServizi.popUpdateDocumento(documento);
            status.setComplete();
            return "redirect:/dirigente/modifyRendicontazionePopIncarico/"
    ...
    public String processDeleteSubmit(@PathVariable("id") int id, @ModelAttribute Documento documento, 
        ...
        this.resppopServizi.popDeleteDocumento(docCmdServizi.findById(id));
        //
        return "redirect:/dirigente/modifyRendicontazionePopIncarico/"
        + documento.getAzione().getObiettivo().getAnno()+"/"
        + documento.getAzione().getObiettivo().getIncaricoID();  
    ...
    public String createDocumentoManager(@PathVariable("idAzione") int idAzione,Model model) {
        ...
        return "geko/rendicontazione/resppop/formDocumentoCreatePop";
    }
    //
    ...
    public String processSubmitDocumentoManager(@ModelAttribute Documento documento,
        ...
        if (result.hasErrors()) {
            ...
            //
            this.resppopServizi.popCreateDocumento(documento);
            //
            status.setComplete();
    ...
    public String editDescrizioneGet(@PathVariable("id") int id, Model model) {
        Documento documento = docCmdServizi.findById(id);
            model.addAttribute("documento", documento);
            return "geko/rendicontazione/resppop/formDocumentoEditDescrizionePop";
    ...
    public String editFileGet(@PathVariable("id") int id, Model model) {
		...
		if (menu.getIdIncaricoScelta() == documento.getIncaricoID()){
	            model.addAttribute("documento", documento);
	            return "geko/rendicontazione/resppop/formDocumentoEditFilePop";
	        }
	        else return "redirect:/ROLE_RESP_POP";
	    }
    //
