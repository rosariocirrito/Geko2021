...
@Controller
@RequestMapping("/controllerValutPdf")
public class ControllerValutPdfController  {
	
	private static final Logger log = LoggerFactory.getLogger(ControllerValutPdfController.class);
	//
    @Autowired
    ...
    //
    public ControllerValutPdfController() { }
    
       
    //----------------------- Valutazione -------------------------------------------------------------------
    // pdf valutazionidipartimento per anno
    @RequestMapping(value="pdfValutazioniDipartimentoController/{anno}/{idIncarico}")
    public void pdfValutazioniDipartimentoController(@PathVariable("anno") int anno, @PathVariable("idIncarico") int idIncarico,
     ...
     // imposto i font
    	...
    	final PersonaGiuridicaGeko dipartimento = fromOrganikoServizi.findPersonaGiuridicaById(incaricoApic.pgID);
    	final String responsabileDept = incaricoApic.getResponsabile();
    	
    	// scrivo sulla prima pagina il titolo del report con il nome della struttura
		Paragraph titoloApic = new Paragraph("Valutazione Dirigenziali per "+dipartimento.getDenominazione()+" - anno:"+anno,fontTitle1);
		doc.add(titoloApic);
		// Riga vuota
	    doc.add(new Paragraph("\n"));
		Paragraph titolo2 = new Paragraph("Dirigente Generale Responsabile "+responsabileDept,fontTitle2);
		doc.add(titolo2);
    	
    	
    	// itero sugli incarichi dipartimentali
    	final List<IncaricoGeko> listIncarichiDept = fromOrganikoServizi.findIncarichiByDipartimentoIDAndAnno(incaricoApic.pgID, anno);
    	for (IncaricoGeko incarico : listIncarichiDept){
    		List<Obiettivo> listObiettivi = objServizi.findObiettiviPrioritariByIncaricoIDAndAnno(incarico.idIncarico, anno);
		    ...
		    if (null != incarico.responsabile) {
			    titolo3 = new Paragraph("RESPONSABILE "+incarico.responsabile);
		    } else{
		    ...
		    titolo3.setAlignment("CENTER");
	     	...
	     	Paragraph titolo4;
		    if (null != incarico.responsabile) {
			    titolo4 = new Paragraph("da: "+incarico.getStrDataInizio()+" a: "+incarico.getStrDataFine());
	    	...
	    	// scheda 1
		    	...
		    	this.generaSchedaValutazObiettivi(doc, incarico, incaricoApic,anno);
		    	//this.addFirme(doc,incaricoApic,inc);
		    	this.generaSchedeValutazDettaglioObiettivi(doc, incarico, incaricoApic,anno);
			    //this.addFirme(doc,incaricoApic,inc);
		    }
    ...
    // pdfValutazioneIncaricoController
    @RequestMapping(value="pdfValutazioneIncaricoController/{anno}/{idIncarico}")
    ...
    // pdfValutazioneIncaricoApicaleController
    @RequestMapping(value="pdfValutazioneApicaleIncaricoController/{anno}/{idIncarico}")
    public void pdfValutazioneApicaleIncaricoController(@PathVariable("anno") int anno, @PathVariable("idIncarico") int idIncarico,
    		HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	// cfr iText in Action p.285 e 289
    	try {
    		String text = request.getParameter("text");
 ...
 // pdf valutazionidipartimento per anno
    @RequestMapping(value="pdfSintesiValutazioniDipartimentoController/{anno}/{idIncarico}")
    public void pdfSintesiValutazioniDipartimentoController(@PathVariable("anno") int anno,  @PathVariable("idIncarico") int idIncarico,
    		HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	// cfr iText in Action p.285 e 289
    	try {
    		String text = request.getParameter("text");
     ...
     // imposto i font
		...
		Map<IncaricoGeko,Float> mapIncarichi = new HashMap<IncaricoGeko,Float>();
		// estraggo le collections che mi servono     
		final IncaricoGeko incaricoApicale = fromOrganikoServizi.findIncaricoById(idIncarico);
    	final PersonaFisicaGeko dirigGen = fromOrganikoServizi.findPersonaFisicaById(incaricoApicale.pfID);
    	final PersonaGiuridicaGeko dept = fromOrganikoServizi.findDipartimentoByPersonaFisicaID(incaricoApicale.pfID);
    	...
    	for (IncaricoGeko inc : listIncarichiDept){
    		List<Obiettivo> lstObjOper = objServizi.findObiettiviPrioritariByIncaricoIDAndAnno(inc.idIncarico, anno);
    		inc.setObiettivi(lstObjOper);	
			//
			...
			for (Valutazione valutazione : lstValutDirig){
				//if (valutazione.getAnno() == anno){
					//System.out.println("pdfSintesiValutazioniDipartimentoController() incarico id: "+incarico.getIdIncarico()+" punteggio tot: "+(Float)valutazione.getTotPunteggio());
					mapIncarichi.put(inc, (Float)valutazione.getTotPunteggio());
				//}
			} // fine for valutazioni
			
		} // fine for incarico
		
		//
		Map<IncaricoGeko,Float> sortedIncarichi = sortByComparator(mapIncarichi);
		...
		// scrivo sulla prima pagina il titolo del report con il nome della struttura
				...
				// itero sugli incarichi dipartimentali
				for (Map.Entry<IncaricoGeko,Float> entry : sortedIncarichi.entrySet()){
					IncaricoGeko incarico = entry.getKey();
					...
					for (Valutazione valutazione : incarico.getValutazioni()){
						...
						if (valutazione.getAnno() == anno){
						    ...
						    tableSintesi.setWidths(new int[] {4,1});
						    // intestazione tabella valutazione
						    PdfPCell cellash1 = new PdfPCell(new Phrase("Incarico: " + incarico.getStringa() + " - anno:" + anno));
						    ...
						    PdfPCell cellash3 = new PdfPCell(new Phrase("Punteggio"));
					    	cellash3.setBackgroundColor(Color.LIGHT_GRAY);
						    cellash3.setHorizontalAlignment(Element.ALIGN_CENTER);
						    tableSintesi.addCell(cellash3);
						    
						    
						    // riga Totale Performance Operativa
						    PdfPCell cellapor1 = new PdfPCell(new Phrase("Totale Performance Operativa (70)"));
						    cellapor1.setBackgroundColor(Color.WHITE);
						    cellapor1.setHorizontalAlignment(Element.ALIGN_LEFT);
						    tableSintesi.addCell(cellapor1);
						    
						    PdfPCell cellapor3 = new PdfPCell(new Phrase(""+(new DecimalFormat("#.##").format(incarico.getTotPunteggioObiettivi()))));
						    cellapor3.setBackgroundColor(Color.WHITE);
						    cellapor3.setHorizontalAlignment(Element.ALIGN_CENTER);
						    tableSintesi.addCell(cellapor3);
						    
						    // Capacità di intercettare, gestire risorse e programmare
						    PdfPCell cellaapr1 = new PdfPCell(new Phrase("Capacità di intercettare, gestire risorse e programmare (range 0, 5-10)"));
						    cellaapr1.setBackgroundColor(Color.WHITE);
						    cellaapr1.setHorizontalAlignment(Element.ALIGN_LEFT);
						    tableSintesi.addCell(cellaapr1);
						    PdfPCell cellaapr4 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getGestPunteggio())));
						    cellaapr4.setBackgroundColor(Color.WHITE);
						    cellaapr4.setHorizontalAlignment(Element.ALIGN_LEFT);
						    tableSintesi.addCell(cellaapr4);
						    
						 // riga Promozione di strumenti di analisi
						    PdfPCell cellarcr1 = new PdfPCell(new Phrase("Promozione di strumenti di analisi e/o adozione di nuove metodologie di semplificazione (range 0, 5-10)"));
						    cellarcr1.setBackgroundColor(Color.WHITE);
						    cellarcr1.setHorizontalAlignment(Element.ALIGN_LEFT);
						    tableSintesi.addCell(cellarcr1);
						    PdfPCell cellarcr4 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getAnalPunteggio())));
						    cellarcr4.setBackgroundColor(Color.WHITE);
						    cellarcr4.setHorizontalAlignment(Element.ALIGN_LEFT);
						    tableSintesi.addCell(cellarcr4);
						    
						    // riga Capacita' di valorizzare competenze ed attitudini 
						    PdfPCell cellagrr1 = new PdfPCell(new Phrase("Capacita' di valorizzare competenze ed attitudini dei propri collaboratori (range 0, 5-10)"));
						    cellagrr1.setBackgroundColor(Color.WHITE);
						    cellagrr1.setHorizontalAlignment(Element.ALIGN_LEFT);
						    tableSintesi.addCell(cellagrr1);
						    PdfPCell cellagrr4 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getRelazPunteggio())));
						    cellagrr4.setBackgroundColor(Color.WHITE);
						    cellagrr4.setHorizontalAlignment(Element.ALIGN_LEFT);
						    tableSintesi.addCell(cellagrr4);
						    
						 // riga Capacità di individuazione del livello di priorità degli interventi da realizzare
						    PdfPCell cellaind1 = new PdfPCell(new Phrase("Capacità di individuazione del livello di priorità degli interventi da realizzare (0, range 0, 5-10)"));
						    cellaind1.setBackgroundColor(Color.WHITE);
						    cellaind1.setHorizontalAlignment(Element.ALIGN_LEFT);
						    tableSintesi.addCell(cellaind1);
						    PdfPCell cellaind2 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getPdlPunteggio())));
						    cellaind2.setBackgroundColor(Color.WHITE);
						    cellaind2.setHorizontalAlignment(Element.ALIGN_LEFT);
						    tableSintesi.addCell(cellaind2);

						    
						    // riga Totale conseguibile Comportamento Organizzativo
						    PdfPCell cellacor1 = new PdfPCell(new Phrase("Totale conseguibile Comportamento Organizzativo (30)"));
						    ...
						    String cellfu ="";
						    if(valutazione.getTotPunteggio() <70){
						    	cellfu = "0%";
						    }
						    else if(valutazione.getTotPunteggio() >=70 && valutazione.getTotPunteggio() <76){
						    	cellfu = "15%";
						    }
						    else if(valutazione.getTotPunteggio() >=76 && valutazione.getTotPunteggio() <81){
						    	cellfu = "20%";
						    }
						    else if(valutazione.getTotPunteggio() >=81 && valutazione.getTotPunteggio() <91){
						    	cellfu = "25%";
						    }
						    else if(valutazione.getTotPunteggio() >=91 && valutazione.getTotPunteggio() <=100){
						    	cellfu = "30%";
						    }

						    // riga Totale Conseguibile
						    PdfPCell cellatcr1 = new PdfPCell(new Phrase("Totale Conseguibile (100) [% indennità]"));
						    ...
						    PdfPCell cellatcr3 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getTotPunteggio())+" ["+cellfu+"]" ));
    ...
    @RequestMapping(value="pdfValutazioneIncaricoControllerSenzaCalcolo/{anno}/{idIncarico}")
    public void pdfValutazioniAltraStrutturaControllerSenzaCalcolo(@PathVariable("anno") int anno, @PathVariable("idIncarico") int idIncarico,
    		HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	// cfr iText in Action p.285 e 289
    	try {
    		String text = request.getParameter("text");
 ...
 // **************************** METODI PRIVATI **********************************************************************
    // -------------------------------------------------------------------------------------
    private void generaSchedaValutazObiettivi(Document doc, IncaricoGeko inc, IncaricoGeko incaricoApicale, int anno){
    	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yy"); 
    	//
		Font fontTitle3 = new Font();
		fontTitle3.setFamily("HELVETICA");
    	...
    	PdfPCell cellriga2 = new PdfPCell(new Paragraph("Dirigenza non apicale"));
    	...
    	PdfPCell cellResp = new PdfPCell(new Paragraph("DIRIGENTE: "+nomeResponsabile));
    	...
    	try {
			tableSk1.setWidths(new int[] {1,5,1,2,2,2,1,1,1});
		} catch (DocumentException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		for (Valutazione valutazione : inc.getValutazioni()){
			...
			if (valutazione.getAnno() == anno){
			    ...
			    PdfPCell cellagrr2 = new PdfPCell(new Phrase(""+valutazione.getGestRealAss()));
			    ...
			    PdfPCell cellagrr3 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getGestPunteggio())));
		...
		this.addFirme(doc, "IL DIRIGENTE GENERALE","IL DIRIGENTE",incaricoApicale.getResponsabile(),inc.getResponsabile());
 ...
 // -------------------------------------------------------------------------------------
    private void generaSchedaValutazObiettiviApicali(Document doc, IncaricoGeko inc, int anno){
	    ...
	    //
	    PdfPCell cellatpo2c = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(inc.getTotPunteggioObiettivi())));
		...
		for (Valutazione valutazione : inc.getValutazioni()){			
			if (valutazione.getAnno() == anno){
			    ...
			    cellagrr3.setHorizontalAlignment(Element.ALIGN_CENTER);
			    tableValutaz.addCell(cellagrr3);			    
			    PdfPCell cellagrr2 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getGestPunteggio())));
			    cellagrr2.setBackgroundColor(Color.WHITE);
			    cellagrr2.setHorizontalAlignment(Element.ALIGN_CENTER);
			    tableValutaz.addCell(cellagrr2);
			    ...
			    cellaapr3.setHorizontalAlignment(Element.ALIGN_CENTER);
			    tableValutaz.addCell(cellaapr3);
			    PdfPCell cellaapr2 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getAnalPunteggio())));
			    cellaapr2.setBackgroundColor(Color.WHITE);
			    cellaapr2.setHorizontalAlignment(Element.ALIGN_CENTER);
			    tableValutaz.addCell(cellaapr2);
			    // riga Relazione e Coordinamento
			    ...
			    cellarcr3.setHorizontalAlignment(Element.ALIGN_CENTER);
			    tableValutaz.addCell(cellarcr3);
			    PdfPCell cellarcr2 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getRelazPunteggio())));
			    cellarcr2.setBackgroundColor(Color.WHITE);
			    cellarcr2.setHorizontalAlignment(Element.ALIGN_CENTER);
			    tableValutaz.addCell(cellarcr2);
			 // riga priorità
			    ...
			    cellar3.setHorizontalAlignment(Element.ALIGN_CENTER);
			    tableValutaz.addCell(cellar3);
			    PdfPCell cellar2 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(valutazione.getPdlPunteggio())));
			    cellar2.setBackgroundColor(Color.WHITE);
			    cellar2.setHorizontalAlignment(Element.ALIGN_CENTER);
			    tableValutaz.addCell(cellar2);
			    try {
			    ...
			    float totCompOrgPunteggio = valutazione.getAnalPunteggio()+
			    		...
			    		valutazione.getGestPunteggio()+
			    		valutazione.getPdlPunteggio();
			    PdfPCell cellacor3 = new PdfPCell(new Phrase(""+new DecimalFormat("#.##").format(totCompOrgPunteggio)));
			    cellacor3.setBackgroundColor(Color.WHITE);
			    cellacor3.setHorizontalAlignment(Element.ALIGN_CENTER);
			    tableSintesi.addCell(cellacor3);
    ...
    // ------------------------------------------------------------------------------------
    private void generaSchedeValutazDettaglioObiettivi(Document doc, IncaricoGeko inc, IncaricoGeko incaricoApicale, int anno){
    	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yy"); 
    	int nordObj = 0;
    	for(Obiettivo obj : inc.getObiettivi()){
    		//
    ...
    // *************************************************
    @RequestMapping(value="pdfValutazioneCompartoController/{anno}/{idIncarico}")
    public void pdfValutazioneCompartoController(@PathVariable("anno") int anno, @PathVariable("idIncarico") int idIncarico,
    		HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	// cfr iText in Action p.285 e 289
    	try {
    		String text = request.getParameter("text");
    ...
    // 4.
    @RequestMapping(value="pdfValutazioneCompartoDipartimentoController/{anno}/{idIncaricoApicale}")
    public void pdfValutazioneCompartoDipartimentoController(@PathVariable("anno") int anno, @PathVariable("idIncaricoApicale") int idIncaricoApicale,
    		HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	// cfr iText in Action p.285 e 289
    	try {
    		String text = request.getParameter("text");
		...
		// estraggo le collections che mi servono 
		final IncaricoGeko incaricoApicale = fromOrganikoServizi.findIncaricoById(idIncaricoApicale);
        final PersonaGiuridicaGeko dipartimento = fromOrganikoServizi.findPersonaGiuridicaById(incaricoApicale.pgID);
        final PersonaFisicaGeko dirigenteGenerale = fromOrganikoServizi.findPersonaFisicaById(incaricoApicale.pfID);
		
     // scrivo sulla prima pagina il titolo del report con il nome della struttura
 		Paragraph titoloMain = new Paragraph("Valutazione Dirigenziale del Personale Comparto per "+dipartimento.getDenominazione()+" - anno:"+anno);
 		doc.add(titoloMain);
 		// Riga vuota
 	    doc.add(new Paragraph("\n"));
 		Paragraph titoloMain2 = new Paragraph("Dirigente Generale"+dirigenteGenerale.stringa);
 		doc.add(titoloMain2);
        
        
        
        final List<IncaricoGeko> listaIncarichiDirigenziali = fromOrganikoServizi.findIncarichiByDipartimentoIDAndAnno(incaricoApicale.pgID,anno);
		
		// itero sugli incarichi dipartimentali
		for (IncaricoGeko incarico: listaIncarichiDirigenziali) {
		
	        final PersonaGiuridicaGeko struttura = fromOrganikoServizi.findPersonaGiuridicaById(incarico.pgID);
	        final PersonaFisicaGeko manager = fromOrganikoServizi.findPersonaFisicaById(incarico.pfID);
	        
			List<PersonaFisicaGeko> listDipendenti = fromOrganikoServizi.findDipendentiStrictByStrutturaIDAndAnno(incarico.pgID, anno);
			if (manager != null && listDipendenti!= null && !listDipendenti.isEmpty() && listDipendenti.contains(manager)) listDipendenti.remove(manager);
			//
			Map<PersonaFisicaGeko,List<AzioneAssegnazione>> mapDipendentiAssegnazioni = new LinkedHashMap();
			Map<PersonaFisicaGeko,List<ValutazioneComparto>> mapDipendentiValutazioneComparto = new LinkedHashMap();
    	...
    	} // fine for incarichi dirigenziali
