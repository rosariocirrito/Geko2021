package it.sicilia.regione.gekoddd.geko.pianificazione.web.qry;
...
@Controller
@RequestMapping("/pianifHtml")
public class PianifHtmlController  {
	
	private Log log = LogFactory.getLog(PianifHtmlController.class);
  ...
  //-----------------------Direttiva --------------------
    @RequestMapping(value="listDirettivaIndirizzo/{anno}", method = RequestMethod.GET)
    public String listDirettivaIndirizzo(@PathVariable("anno") int anno, Model model) {
  ...
  //-----------------------Direttiva --------------------
    @RequestMapping(value="listDirettivaAzAmmin/{anno}", method = RequestMethod.GET)
    public String listDirettivaAzAmmin(@PathVariable("anno") int anno, Model model) {
    	List<AreaStrategica> listAreeStrategiche = areaStrategicaQryServizi.findByAnno(anno);
    	//
        for (AreaStrategica area : listAreeStrategiche){
        	List<ObiettivoStrategico> lstobjstr = objStratServizi.findByAreaStrategicaAndAnno(area, anno);
        		...
        		for(ObiettivoStrategico stra : lstobjstr){
        			if(!stra.getAssociazObiettiviApicali().isEmpty()){
                                    ...
                                    apicale.setResponsabile(inc.getDenominazioneDipartimento()
                                                    +" / "+inc.denominazioneStruttura+ " - "+inc.responsabile); 
                                    }
        			}
        		}
        	}	
  ...
  //-----------------------PianificazioneTriennale --------------------
    @RequestMapping(value="listPianificazioneTriennale/{anno}", method = RequestMethod.GET)
    public String listPianificazioneTriennale(@PathVariable("anno") int anno, Model model) {
    	List<AreaStrategica> listAreeStrategiche = areaStrategicaQryServizi.findByAnno(anno);
    	int annoInizio = anno;
    	int annoFine = anno+3;
    	 //
